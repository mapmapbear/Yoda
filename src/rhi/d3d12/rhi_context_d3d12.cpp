...
size_t RHIContextD3D12::initialize(HWND handle) {
	...
	if (status != 0) {
		Logger::get_singleton().singletonLogger->error("device factory create error !!!");
		return -1;
	}
#ifdef DEBUG
	status = init_debug_layers();
	if (status != 0) {
		Logger::get_singleton().singletonLogger->error("debug layer create error !!!");
		return -1;
	}
#endif
	status = init_devices();
...
size_t RHIContextD3D12::init_device_factory() {
	...
	if (SUCCEEDED(d3d_D3D12GetInterface(CLSID_D3D12SDKConfiguration, IID_PPV_ARGS(&sdk_config)))) {
		...
		if (SUCCEEDED(sdk_config->QueryInterface(&sdk_config1))) {
			if (SUCCEEDED(sdk_config1->CreateDeviceFactory(agility_sdk_version, agility_sdk_path.data(), IID_PPV_ARGS(device_factory.GetAddressOf())))) {
...
size_t RHIContextD3D12::init_debug_layers() {
	...
	if (device_factory) {
		...
		if (d3d_D3D12GetDebugInterface == nullptr) {
			Logger::get_singleton().singletonLogger->error("can't not get d3d12 debug layer interface !!!");
			return -1;
		}
...
size_t RHIContextD3D12::init_devices() {
	...
	if (dxgi_CreateDXGIFactory2 == nullptr) {
		Logger::get_singleton().singletonLogger->error("can't not load CreateDXGIFactory Func Address !!!");
		return -1;
	}

	HRESULT res = dxgi_CreateDXGIFactory2(dxgi_factory_flags, IID_PPV_ARGS(&dxgi_factory));
	if (!SUCCEEDED(res)) {
		Logger::get_singleton().singletonLogger->error("can't not Create DXGIFactory !!!");
		return -1;
	}
    ...
    if (FAILED(res))
        Logger::get_singleton().singletonLogger->error("D3D12: Failed to query debug device!");
    ...
    // if don't need adapter devce infomation, please release "adapter device array" !!!!
